    ...
    def train(self, data,test_data, log,sample_interval=1):
        ...
        for epoch in range(config.NUM_EPOCHS):
                for batch in range(total_batch):
                    ...
                    # update log files
                    write_log(None, self.train_names, g_loss, (epoch*total_batch+batch+1))
                    write_log(None, self.disc_names, d_loss, (epoch*total_batch+batch+1))
...
if __name__ == '__main__':
    wandb.init(project="pmldl-colorization")
    ...
    with open(os.path.join(log_path, str(datetime.datetime.now().strftime("%Y%m%d")) + "_" + str(config.BATCH_SIZE) + "_" + str(config.NUM_EPOCHS) + ".txt"), "w") as log:
        ...
        model_path = os.path.join(config.MODEL_DIR, config.PRETRAINED)
        colorizationModel = MODEL()
        colorizationModel.colorizationModel = load_model(model_path)
